...
pub async fn list_images(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // Values are all strings so we use Vec<String> which is Send.
...
#[tauri::command]
pub async fn get_image_detail(id: i64, state: State<'_, AppState>) -> Result<ImageDetail, String> {
    let conn = state.conn.clone();
...
#[tauri::command]
pub async fn list_directories(state: State<'_, AppState>) -> Result<Vec<DirectoryEntry>, String> {
    let conn = state.conn.clone();
...
#[tauri::command]
pub async fn get_filter_options(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let conn = state.conn.clone();
...
#[tauri::command]
pub async fn get_object_options(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    let conn = state.conn.clone();
